...
# Step-by-step connection guide for the bastion tunnel workflow.
# Includes every command needed from open tunnel to connected DB client.
# Re-run `terraform output bastion_connection_guide` after restarting the
...
output "bastion_connection_guide" {
  ...
  value = one([
    ...
    Step 4 — Stop the bastion when done to avoid charges (~$0.005/hr running):
      aws ec2 stop-instances --instance-ids ${b.instance_id} --region ${var.aws_region}
    GUIDE
  ])
}
